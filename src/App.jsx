...
SOLO JSON: {"mes1":{"min":100,"max":400,"descripcion":""},"mes3":{"min":400,"max":1500,"descripcion":""},"mes6":{"min":1000,"max":4000,"descripcion":""},"mes12":{"min":3000,"max":12000,"descripcion":""},"estrategia":["acción1","acción2","acción3"],"ventaja_pais":"ventaja","perfil":"1 oración","potencial":"bajo|medio|alto|muy_alto"}`;
  ...
  if (result && !result.error) return (
    <div style={{padding:"20px"}}>
      ...
      <div style={{marginBottom:9}}>{[{k:"mes1",l:"1 mes"},{k:"mes3",l:"3 meses"},{k:"mes6",l:"6 meses"},{k:"mes12",l:"12 meses"}].map(({k,l})=>{const d=result[k];if(!d)return null;return(<div key={k} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:1}}><span style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.55)"}}>{l}</span><span style={{fontFamily:"'Inter',sans-serif",fontSize:10,fontWeight:900,color:"#C9A84C"}}>${d.min?.toLocaleString()} – ${d.max?.toLocaleString()}</span></div><div style={{height:4,background:"rgba(255,255,255,0.08)",borderRadius:2}}><div style={{height:"100%",width:`${Math.min((d.max/12000)*100,100)}%`,background:"linear-gradient(90deg,#C9A84C,#FFD700)",borderRadius:2}}/></div></div>);})}</div>
      <div style={{background:"rgba(201,168,76,0.1)",border:"1px solid rgba(201,168,76,0.2)",borderRadius:9,padding:"7px 9px",marginBottom:8}}><p style={{fontFamily:"'Inter',sans-serif",fontSize:9,color:"rgba(255,255,255,0.7)",lineHeight:1.5}}>{result.ventaja_pais}</p></div>
      <div style={{marginBottom:9}}>{result.estrategia?.map((s,i)=><div key={i} style={{display:"flex",gap:6,marginBottom:4}}><div style={{width:15,height:15,borderRadius:"50%",background:"linear-gradient(135deg,#C9A84C,#FFD700)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif",fontSize:8,fontWeight:900,flexShrink:0,color:"#1a1a1a"}}>{i+1}</div><span style={{fontFamily:"'Inter',sans-serif",fontSize:9,color:"rgba(255,255,255,0.7)",lineHeight:1.5}}>{s}</span></div>)}</div>
      <div style={{display:"flex",gap:7}}>
  ...
  return (
    <div style={{padding:"20px"}}>
      <div style={{textAlign:"center",marginBottom:14}}><div style={{fontSize:28}}>💰</div><h3 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:14,fontWeight:700,color:"#FFD700",margin:"4px 0 2px"}}>ROI Predictor IA</h3><p style={{fontFamily:"'Inter',sans-serif",fontSize:10,color:"rgba(255,255,255,0.5)"}}>Proyección de ingresos con IA real</p></div>
      <div style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.55)",marginBottom:4}}>PAÍS</div><select value={form.country} onChange={e=>set("country",e.target.value)} style={{width:"100%",background:"rgba(255,255,255,0.07)",border:"1px solid rgba(201,168,76,0.22)",borderRadius:9,padding:"8px 10px",fontFamily:"'Inter',sans-serif",color:form.country?"#fff":"rgba(255,255,255,0.35)",fontSize:11,outline:"none",cursor:"pointer",boxSizing:"border-box"}}><option value="">Seleccioná tu país...</option>{COUNTRIES.map(c=><option key={c} value={c} style={{color:"#1a1a1a"}}>{c}</option>)}</select></div>
      {[{label:"HORAS / SEMANA",opts:hours,key:"hours"},{label:"RED DE CONTACTOS",opts:contacts,key:"contacts"},{label:"EXPERIENCIA",opts:experience,key:"experience"}].map(({label,opts,key})=><div key={key} style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.55)",marginBottom:4}}>{label}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>{opts.map(o=><button key={o} onClick={()=>set(key,o)} style={{padding:"6px 4px",borderRadius:7,fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:9,cursor:"pointer",border:"1.5px solid",borderColor:form[key]===o?"#C9A84C":"rgba(255,255,255,0.1)",background:form[key]===o?"rgba(201,168,76,0.18)":"rgba(255,255,255,0.04)",color:form[key]===o?"#C9A84C":"rgba(255,255,255,0.6)",transition:"all .2s"}}>{o}</button>)}</div></div>)}
...
3 fases. Productos FuXion reales.`;
  ...
  if (step===1) return (
    <div style={{padding:"20px"}}>
      <div style={{textAlign:"center",marginBottom:12}}><div style={{fontSize:28}}>👑</div><h3 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:14,fontWeight:700,color:"#FFD700",marginBottom:2}}>Elite Program</h3><p style={{fontFamily:"'Inter',sans-serif",fontSize:10,color:"rgba(255,255,255,0.45)"}}>La IA genera tu protocolo de 90 días</p></div>
      <div style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:3}}>NOMBRE</div><input value={form.name} onChange={e=>set("name",e.target.value)} placeholder="¿Cómo te llamás?" style={inp} onFocus={e=>e.target.style.borderColor="#C9A84C"} onBlur={e=>e.target.style.borderColor="rgba(201,168,76,0.22)"}/></div>
      <div style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:3}}>PAÍS</div><select value={form.country} onChange={e=>set("country",e.target.value)} style={{...inp,cursor:"pointer",color:form.country?"#fff":"rgba(255,255,255,0.35)"}}><option value="">Seleccioná tu país...</option>{COUNTRIES.map(c=><option key={c} value={c} style={{color:"#1a1a1a"}}>{c}</option>)}</select></div>
      <div style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:4}}>OBJETIVO</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>{goals.map(g=><button key={g} onClick={()=>set("goal",g)} style={{padding:"6px 4px",borderRadius:7,fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:8,cursor:"pointer",border:"1.5px solid",borderColor:form.goal===g?"#C9A84C":"rgba(255,255,255,0.1)",background:form.goal===g?"rgba(201,168,76,0.18)":"rgba(255,255,255,0.04)",color:form.goal===g?"#FFD700":"rgba(255,255,255,0.55)",transition:"all .2s"}}>{g}</button>)}</div></div>
      <div style={{marginBottom:8}}><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:4}}>SÍNTOMAS</div><div style={{display:"flex",flexWrap:"wrap",gap:3}}>{symptoms.map(s=>{const sel=form.symptoms.includes(s);return <button key={s} onClick={()=>set("symptoms",sel?form.symptoms.filter(x=>x!==s):[...form.symptoms,s])} style={{padding:"2px 6px",borderRadius:999,fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:8,cursor:"pointer",border:"1.5px solid",borderColor:sel?"#C9A84C":"rgba(255,255,255,0.1)",background:sel?"rgba(201,168,76,0.15)":"transparent",color:sel?"#C9A84C":"rgba(255,255,255,0.45)",transition:"all .2s"}}>{s}</button>;})}</div></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}>
        <div><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:3}}>EDAD</div><input value={form.age} onChange={e=>set("age",e.target.value)} placeholder="32" style={inp}/></div>
        <div><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:700,color:"rgba(255,255,255,0.5)",marginBottom:3}}>PESO kg</div><input value={form.weight} onChange={e=>set("weight",e.target.value)} placeholder="70" style={inp}/></div>
      </div>
  ...
  if (step===2) return (
    <div style={{padding:"40px 20px",textAlign:"center"}}>
      ...
      <p style={{fontFamily:"'Inter',sans-serif",fontSize:10,color:"rgba(255,255,255,0.45)",marginBottom:16,lineHeight:1.6}}>Claude AI analiza tu perfil y construye tu plan único de 90 días</p>
  ...
  if (step===3 && plan) return (
    <div style={{padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        <button onClick={openWA} style={{width:"100%",background:"linear-gradient(135deg,#C9A84C,#FFD700)",color:"#fff",border:"none",borderRadius:10,padding:"12px 0",fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:12,cursor:"pointer"}}>👑 Quiero este plan · Ver precio en {form.country} →</button>
        <div style={{display:"flex",gap:6}}><button onClick={()=>{setStep(1);setPlan(null);}} style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.6)",borderRadius:9,padding:"7px 0",fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:10,cursor:"pointer"}}>🔄 Nuevo</button><button onClick={()=>{const txt=`${plan.titulo}\nFase 1: ${plan.fases?.[0]?.nombre}\nFase 2: ${plan.fases?.[1]?.nombre}\nFase 3: ${plan.fases?.[2]?.nombre}`;navigator.clipboard?.writeText(txt);}} style={{flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",color:"rgba(255,255,255,0.6)",borderRadius:9,padding:"7px 0",fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:10,cursor:"pointer"}}>📋 Copiar</button></div>
      </div>
    </div>
  );
  return null;
  ...
  return (
    <div style={{background:"#fff",border:"1.5px solid rgba(45,106,79,0.1)",borderRadius:20,overflow:"hidden",boxShadow:"0 6px 24px rgba(45,106,79,0.07)"}}>
      <div style={{background:"linear-gradient(135deg,#1a2e1a,#2d6a4f)",padding:"13px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:12,fontWeight:700,color:"#fff",marginBottom:1}}>Actividad en tiempo real</div><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,color:"rgba(255,255,255,0.5)"}}>Últimas 24 horas · Global</div></div>
...
function AntesDespues() {
  ...
  return (
    <div style={{background:"#fff",border:"1.5px solid rgba(45,106,79,0.1)",borderRadius:20,overflow:"hidden",boxShadow:"0 6px 24px rgba(45,106,79,0.07)"}}>
      <div style={{background:"linear-gradient(135deg,#1a2e1a,#2d6a4f)",padding:"13px 16px"}}><div style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:12,fontWeight:700,color:"#fff",marginBottom:1}}>Transformaciones reales</div><div style={{fontFamily:"'Inter',sans-serif",fontSize:9,color:"rgba(255,255,255,0.5)"}}>Casos verificados</div></div>
  ...
  return (
    <div style={{position:"fixed",bottom:84,right:20,width:295,zIndex:300,background:"rgba(250,250,247,0.97)",backdropFilter:"blur(20px)",border:"1.5px solid rgba(45,106,79,0.22)",borderRadius:18,boxShadow:"0 20px 52px rgba(45,106,79,0.2)",overflow:"hidden"}}>
      <div style={{background:"#1a2e1a",padding:"11px 13px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:26,height:26,borderRadius:"50%",background:"rgba(201,168,76,0.28)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🌿</div><div><div style={{fontFamily:"'Playfair Display',Georgia,serif",color:"#fff",fontWeight:700,fontSize:12}}>Vita Advisor IA</div><div style={{fontFamily:"'Inter',sans-serif",color:"rgba(255,255,255,0.5)",fontSize:9}}>● Online</div></div></div>
        <button onClick={onClose} style={{background:"none",border:"none",color:"rgba(255,255,255,0.65)",fontSize:15,cursor:"pointer"}}>✕</button>
      </div>
    ...
    <div style={{fontFamily:"Georgia,serif",background:"#FAFAF7",color:"#1a2e1a",overflowX:"hidden"}}>
      ...
      <div style={{background:"#1a2e1a",padding:"12px 0",overflow:"hidden"}}>
        <div style={{display:"flex",gap:48,animation:"marquee 28s linear infinite",width:"max-content"}}>
          {certDouble.map((c,i)=><span key={i} className="int" style={{color:"rgba(255,255,255,0.4)",fontSize:10,fontWeight:700,letterSpacing:2,textTransform:"uppercase",whiteSpace:"nowrap"}}>✦ {c}</span>)}
        </div>
      </div>
              ...
              <div key={p.id} onMouseEnter={()=>setHovCard(p.id)} onMouseLeave={()=>setHovCard(null)} style={{background:"#fff",borderRadius:22,overflow:"hidden",border:`1px solid ${hovCard===p.id?p.color+"45":"rgba(0,0,0,0.06)"}`,boxShadow:hovCard===p.id?`0 18px 48px ${p.color}20`:"0 3px 14px rgba(0,0,0,0.05)",transition:"all .3s",transform:hovCard===p.id?"translateY(-5px)":"translateY(0)",cursor:"pointer",display:"flex",flexDirection:"column"}}>
                ...
                <div style={{padding:"17px 17px 14px",flex:1,display:"flex",flexDirection:"column",gap:6}}>
                  ...
                  <div style={{display:"flex",gap:6,marginTop:"auto"}}>
                    ...
                    <a href={FUXION_LINK} target="_blank" rel="noreferrer" style={{flex:1,background:p.color,color:"#fff",borderRadius:9,padding:"8px 0",fontFamily:"'Inter',sans-serif",fontWeight:600,fontSize:10,textAlign:"center",textDecoration:"none"}}>🛒 Comprar</a>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section id="iatools" style={{padding:"80px 40px",background:"linear-gradient(160deg,#020b18,#041a0e 50%,#020b18)",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{maxWidth:1200,margin:"0 auto",position:"relative",zIndex:1}}>
          <div style={{textAlign:"center",marginBottom:60}}>
            ...
            <p className="int" style={{color:"rgba(255,255,255,0.48)",fontSize:14,maxWidth:480,margin:"0 auto",lineHeight:1.8}}>4 herramientas que <span style={{color:"#FFD700",fontWeight:700}}>ningún otro distribuidor FuXion</span> en el mundo tiene. Gratis.</p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(290px,1fr))",gap:22}}>
            ...
            <div style={{background:"rgba(18,12,3,0.88)",backdropFilter:"blur(20px)",border:"1px solid rgba(201,168,76,0.3)",borderRadius:24,overflow:"hidden",boxShadow:"0 24px 52px rgba(0,0,0,0.45)",transition:"transform .3s,box-shadow .3s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-8px)";e.currentTarget.style.boxShadow="0 36px 72px rgba(0,0,0,0.55),0 0 100px rgba(201,168,76,0.15)";}} onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="0 24px 52px rgba(0,0,0,0.45)";}}>
              ...
              <div style={{padding:"15px 17px 10px",display:"flex",alignItems:"center",gap:10,borderBottom:"1px solid rgba(201,168,76,0.14)"}}><div style={{width:38,height:38,borderRadius:10,background:"rgba(201,168,76,0.28)",border:"1px solid rgba(255,215,0,0.36)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17}}>💰</div><div style={{flex:1}}><div className="pf" style={{color:"#FFD700",fontWeight:700,fontSize:12,marginBottom:1}}>ROI Predictor IA</div><div className="int" style={{color:"rgba(255,255,255,0.42)",fontSize:9}}>Proyección · Ingresos reales</div></div><div style={{background:"rgba(255,215,0,0.13)",border:"1px solid rgba(255,215,0,0.45)",borderRadius:7,padding:"2px 7px",fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:800,color:"#FFD700"}}>AI ✦</div></div>
              <ROIPredictor/>
            </div>
            {/* Elite Program */}
            <div style={{background:"rgba(10,5,1,0.9)",backdropFilter:"blur(20px)",border:"1px solid rgba(201,168,76,0.45)",borderRadius:24,overflow:"hidden",boxShadow:"0 24px 52px rgba(0,0,0,0.5),0 0 70px rgba(201,168,76,0.07)",transition:"transform .3s,box-shadow .3s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-8px)";e.currentTarget.style.boxShadow="0 36px 72px rgba(0,0,0,0.6),0 0 120px rgba(201,168,76,0.18)";}} onMouseLeave={e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="0 24px 52px rgba(0,0,0,0.5),0 0 70px rgba(201,168,76,0.07)";}}>
              ...
              <div style={{padding:"15px 17px 10px",display:"flex",alignItems:"center",gap:10,borderBottom:"1px solid rgba(201,168,76,0.18)"}}><div style={{width:38,height:38,borderRadius:10,background:"rgba(201,168,76,0.36)",border:"1px solid rgba(255,215,0,0.48)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>👑</div><div style={{flex:1}}><div className="pf" style={{color:"#FFD700",fontWeight:700,fontSize:12,marginBottom:1}}>Elite Program 90 días</div><div className="int" style={{color:"rgba(255,255,255,0.42)",fontSize:9}}>Plan completo · IA genera tu protocolo</div></div><div style={{background:"rgba(255,215,0,0.13)",border:"1px solid rgba(255,215,0,0.55)",borderRadius:7,padding:"2px 7px",fontFamily:"'Inter',sans-serif",fontSize:9,fontWeight:800,color:"#FFD700"}}>AI ✦</div></div>
              <EliteProgram/>
            </div>
          </div>
          <div style={{textAlign:"center",marginTop:40}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:14,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:14,padding:"12px 24px",flexWrap:"wrap",justifyContent:"center"}}>
              {[["🔒","100% Privado"],["⚡","Respuesta inmediata"],["🌿","Solo FuXion"],["🌍","37 países"]].map(([ic,lb],i)=>(<div key={i} style={{display:"flex",alignItems:"center",gap:5}}>{i>0&&<div style={{width:1,height:18,background:"rgba(255,255,255,0.08)"}}/>}<span style={{fontSize:13}}>{ic}</span><span className="int" style={{fontSize:10,color:"rgba(255,255,255,0.38)",fontWeight:600}}>{lb}</span></div>))}
            </div>
          </div>
        </div>
      </section>
